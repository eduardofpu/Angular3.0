...
public class FileUploadService<T> extends GenericService<FileUpload, Long> {
    ...
    public FileUpload insert(@RequestBody FileUpload f) {
		this.LOGGER.debug(String.format("Saving the entity [%s].",f));  // Salvando a entidade.    	
    	f.setId(null);
	...
	public void delete(@RequestBody FileUpload f) {
		this.LOGGER.debug(String.format("Request to delete the record [%s].", f));// Pedido para excluir o registro.
